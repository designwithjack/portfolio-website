...
**The package name is `motion-v`, not `motion`.** This was a mistake made
during setup and is worth remembering — AI tools will sometimes suggest
`motion` or `motion/vue` as the import, both of which are wrong. Always
verify package names against the official docs before installing.
...
This auto-imports most Motion components. The `<motion />` component itself
is not auto-imported — import it manually in any component that uses it:
...
AI tools — including Cursor and Claude — can confidently suggest incorrect
package names. The motion-v vs motion mistake is a good example. Before
running any install command suggested by an AI, check the package name
against the official documentation or npmjs.com.
...
- [ ] Install `@nuxt/content` when ready to start building
- [ ] Add `motion-v/nuxt` to modules in `nuxt.config.ts` ✓ done

## AI verification habit

Before acting on any AI suggestion involving:

- package names or install commands
- import paths
- nuxt.config.ts module names
- version numbers

Check against the official docs or npmjs.com first.
